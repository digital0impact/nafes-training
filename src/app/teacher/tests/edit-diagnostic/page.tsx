...
import { getPrebuiltDiagnosticTests, getQuestionsForModel, type TestModel } from "@/lib/test-models";
...
export default function EditDiagnosticTestPage() {
  ...
  useEffect(() => {
    ...
    if (modelId) {
      ...
      const allModels = getPrebuiltDiagnosticTests();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        router.push("/teacher/tests/diagnostic-models");
      }, 2000);
    } catch (error) {
      console.error(error);
      setStatus("error");
      ...
      <main className="relative min-h-screen overflow-hidden bg-[#faf9f7]">
        ...
        <div className="relative z-10 p-4 py-8">
          <div className="card text-center">
            ...
            <Link
              href="/teacher/tests/diagnostic-models"
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#faf9f7]">
      ...
      <div className="relative z-10 space-y-6 p-4 py-8">
        ...
        <header className="card bg-gradient-to-br from-white to-purple-50">
          ...
          <h1 className="text-3xl font-bold text-slate-900">✏️ تعديل النموذج التشخيصي</h1>
        ...
        <div className="card space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-purple-300 focus:bg-white focus:outline-none"
              value={testTitle}
          ...
          {questions.length === 0 ? (
            ...
            <div className="space-y-6">
              {questions.map((question, index) => (
                <div
                  key={question.id}
                  className="rounded-3xl border-2 border-purple-200 bg-purple-50 p-6"
                >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <textarea
                        className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-purple-300 focus:outline-none"
                        rows={2}
                        value={question.text}
        ...
        <div className="flex gap-3">
          <button
            onClick={handleSave}
            className="flex-1 rounded-2xl bg-purple-600 py-3 text-sm font-semibold text-white disabled:opacity-60 hover:bg-purple-700"
